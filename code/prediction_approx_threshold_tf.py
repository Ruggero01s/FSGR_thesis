...
def analyze_prediction_distributions(model, data_generator, num_samples=100):
    """
...
def analyze_test_set(model, test_set, dizionario, dizionario_goal, 
                     max_dim, perc_action, num_samples, 
    ...
    true_positives, true_negatives = analyze_prediction_distributions(
        model, gen, num_samples=min(num_samples, len(gen))
    )
...
@click.option('--max-dim', type=int, default=32,
...
def main(model_path, model_number, dict_dir, test_plans_dir, num_samples, 
    ...
    This script loads a trained TensorFlow model and evaluates its predictions on all available test data
    ...
    # Load model
    model_file = path.join(model_path, f"model_{model_number}.keras")
    try:
